...
curl --location --request GET 'http://127.0.0.1:8000/memes'


# Execute the POST /memes endpoint using curl

curl --location --request POST 'http://127.0.0.1:8000/memes/' --header 'Content-Type: application/json' --data-raw
'{"name": "testeing", "url": "https://api.memegen.link/images/keanu.jpg?width=300", "caption": "testingCAP" }'
